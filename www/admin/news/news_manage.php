...
//初始化搜索下拉列表
  $strSQL="SELECT id_newsdir,name,ordernum FROM newsdir WHERE dele='1' and level='2' and lang=0 order by ordernum asc";
  $objDB->Execute($strSQL);
  $arr2dirsearch=$objDB->GetRows();
  
  $strSQL="SELECT id_newsdir,name,ordernum FROM newsdir WHERE dele='1' and level='2' and lang=1 order by ordernum asc";
  $objDB->Execute($strSQL);
