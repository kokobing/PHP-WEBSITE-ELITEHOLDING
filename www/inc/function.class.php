<?php
//版本日期：2012-06-30
//if($_SERVER['HTTP_HOST']=='demo-cs6.vicp.net:8080' || $_SERVER['HTTP_HOST']=='demo-cs16.vicp.net'){$webcoming=1;}else{$webcoming=0;}
//if($webcoming==0){echo 'Error message: The current access to the domain name and authorization binding domain are inconsistent, please check your domain name!';ob_end_flush();exit();};
...
//取出指定ID的新闻目录二级目录
function getonenewsdir2($ndir){
global	 $objDB;//函数调用 声明全局变量
$strSQL="SELECT name FROM newsdir WHERE id_newsdir='".$ndir."'";
$objDB->Execute($strSQL);
$getonenewsdir2=$objDB->fields();
return $getonenewsdir2[name];
}
...
//取出新闻图片  指定id  指定postion位置  的一张图片   直接返回文件名
function getonenewspic($nid,$postion){
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select opicname as pic from newspic where id_newsinfo ='".$nid."' order by  id_newspic asc limit $postion,1" ;
$objDB->Execute($strSQL);
$arronepic=$objDB->fields();
return $arronepic[pic];
}
...
//取出指定目录ID的新闻多条列表  指定tableField字段  按asc排序  返回变量数组 引用时需要赋值 再输出
function getnewslistasc($dirid,$tableField){
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select $tableField from newsinfo where dele=1 and id_newsdir=$dirid  order by ordernum asc " ;
$objDB->Execute($strSQL);
$getnewslistasc=$objDB->GetRows();
return $getnewslistasc;
}
